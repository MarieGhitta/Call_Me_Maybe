...
class Constrainator:
    ...
    def _select_function(self, out, user_prompt):
        ...
        prompt = """
                ...
                Your task is to choose exactly one function that best matches the user request.
                Rules:
        ...
        while True:
            ...
            logits = np.array(self.model.get_logits_from_input_ids(entry_model))
    ...
    class TokensCollection:
        def __init__(self, vocab):
            ...
            self.close_bracket = self._tokensif(lambda token: token.count("}") == 1)
            self.numbers = self._tokensif(lambda token: all(char in self.numeric_chars for char in token))
    ...
    def _get_number(self, out, context_tokens, is_last):
        for _ in range(30):
            ...
            logits = np.array(self.model.get_logits_from_input_ids(model_input))
            mask = np.ones(len(logits), dtype=bool)
    ...
    def _get_args(self, function_definition, out, context_tokens):
        ...
        for i, param in enumerate(param_names):
            ...
            if param_type == "number":
                ...
                generated_args[param] = self._get_number(out, context_tokens, is_last)
            elif param_type == "string":
                arg_prefix = f'"{param}":"'
                arg_tokens = self.model.encode(arg_prefix).tolist()[0]
                out.append(arg_tokens)
                generated_value = self._get_string(out, context_tokens, is_last)
                if param == "regex":
